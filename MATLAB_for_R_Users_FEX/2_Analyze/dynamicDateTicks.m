...
% ------------ End of dynamicDateTicks-----------------------
    ...
    function updateDateLabel(obj, ev, varargin)
        ...
        % Address a bug in datetick that draws too few ticks if the date
        % span is exactly a month or year
        if length(ticks) == 2
            try %#ok<TRYNC> If call to dateTickPicker fails, move on
                [ticks, tickFormat] = dateTickPicker(ax1, xlim(ax1), [], 'yqmwdHMS', ax1);
                labels = datestr(ticks, tickFormat);
            end
        end
